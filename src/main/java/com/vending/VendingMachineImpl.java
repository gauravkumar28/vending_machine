...
public class VendingMachineImpl implements VendingMachine {
	...
	private Inventory<Cash> getChange(Integer amount) throws InsufficientChangeException {
		...
		if (amount > 0) {
			...
			while (balance > 0) {
				if (balance >= Cash.HUNDRED_RUPEES.getDenomination() && changesCash
						.getQuantity(Cash.HUNDRED_RUPEES) > cashInventory.getQuantity(Cash.HUNDRED_RUPEES)) {
				...
				} else if (balance >= Cash.TWENTY_RUPEES.getDenomination() && changesCash
						.getQuantity(Cash.TWENTY_RUPEES) > cashInventory.getQuantity(Cash.TWENTY_RUPEES)) {
